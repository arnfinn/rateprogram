...
/*Tilpasning til Â lage Norge som s¯yle*/
PROC SQL;
...
Norge=rateSnitt;
run; 
/*Slutt tilpasning til Â lage Norge som s¯yle*/
%mend omraade;
...
hbarparm category=&bo response=rateSnitt / fillattrs=(color=CX95BDE6); 
     Refline &Norge / axis=x lineattrs=(Thickness=.5 color=Black pattern=2) name="Ref1";
			%if &Antall_aar>1 and &aarsobs=1 %then %do; scatter x=rate&Âr1 y=&Bo / markerattrs=(symbol=squarefilled color=black size=5);%end;
     ...
     inset (
		%if &Antall_aar>1 and &aarsobs=1 %then %do;"(*ESC*){unicode'25a0'x}"="   &Âr1" %end;  
...
/*     Refline &Norge / axis=x lineattrs=(Thickness=.5 color=Black pattern=2) name="Ref1";*/
			%if &Antall_aar>1 and &aarsobs=1 %then %do; scatter x=rate&Âr1 y=&Bo / markerattrs=(symbol=squarefilled color=black);%end;
